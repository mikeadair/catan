...
// Dedicated config for the latency-fuzz suite (web/e2e/latency-fuzz.spec.ts), invoked via the
// `test:e2e:latency` npm script — deliberately NOT just `playwright.config.ts` + a CLI file
// argument. playwright.config.ts's `testIgnore` excludes latency-fuzz.spec.ts from that
...
export default defineConfig({
  testDir: './e2e',
  testMatch: '**/latency-fuzz.spec.ts',
  fullyParallel: false,
  workers: 1,
  retries: process.env.CI ? 1 : 0,
  reporter: [['list'], ['html', { open: 'never', outputFolder: 'playwright-report-latency' }]],
  timeout: 60_000, // per-test test.setTimeout(...) calls in the spec itself override this as needed
  use: {
    baseURL,
