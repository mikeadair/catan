...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 5.A | Fix Functions-emulator crash in CI `capture-screenshots` job | done | Root cause confirmed infrastructure, not app code (see below), so rather than chase an opaque emulator bug further, replaced the Functions emulator for e2e purposes with `functions/src/e2eBridge.ts` — a minimal HTTP server implementing just enough of the callable-functions wire protocol to serve `startGameHandler`/`submitActionHandler` directly (same handlers `npm run test -w functions` already exercises reliably). `web/e2e/run-with-bridge.sh` starts it alongside Playwright; `--only auth,firestore` (functions dropped) in `web/package.json`'s `test:e2e`/`test:e2e:ui`. Verified locally: all 12 tests pass across all 3 viewports in ~42s, and the real screenshot files regenerated correctly (including `game-mid.png`, which had never successfully generated before). Never bundled into the deployed function. |
| 5.B | (superseded investigation, kept for context) | — | Originally reproduced locally (same sandbox class as CLAUDE.md's WSL2 note) with `firebase-tools --debug`: the Functions emulator's worker process for `startGame` died with only a generic "killed because it raised an unhandled error" — no stack trace surfaced through firebase-tools' own logging even in `--debug` mode. `npm run test -w functions` (same handlers, direct call, no Functions Framework HTTP layer) passed 100% throughout — confirmed this was an emulator/worker-process infrastructure issue, not application logic. The previously-noted workaround (`firebase-tools` 15.x needs Java 21+) was inconclusive to verify locally (only Java 11 available) — superseded by 5.A's bridge instead of pursuing that further. |
